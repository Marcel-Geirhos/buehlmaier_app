...
class _NewAssignmentPageState extends State<NewAssignmentPage> {
  ...
  void createNewAssignment(BuildContext context) async {
    if (_consumerName.text.isEmpty) {
      ...
      if (await doesAssignmentAlreadyExist()) {
        createNewAssignmentInCloudFirestore();
      } else {
        _showCheckAssignmentDialog(context);
      }
    }
  }

  createNewAssignmentInCloudFirestore() async {
  ...
  Future<bool> doesAssignmentAlreadyExist() async {
    QuerySnapshot assignments = await Firestore.instance.collection('assignments').getDocuments();
    for (int i = 0; i < assignments.documents.length; i++) {
      if (assignments.documents.elementAt(i).data['Name'].toLowerCase() == _consumerName.text.toLowerCase() &&
          assignments.documents.elementAt(i).data['OrderType'] == _currentOrderType) {
        return false;
      }
    }
    return true;
  ...
  void _showCheckAssignmentDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: Center(child: Text('Der Auftrag für ${_consumerName.text.toString()} mit $_currentOrderType wurde bereits erstellt. Auftrag trotzdem erstellen?')),
          actions: <Widget>[
            FlatButton(
              child: Text('Ja'),
              onPressed: () {
